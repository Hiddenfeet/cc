  ...
  return useMemo(
    () =>
      addresses.reduce<{ [address: string]: CurrencyAmount<Native> }>((memo, address, i) => {
        ...
        if (value)
          memo[address.toLocaleLowerCase()] = CurrencyAmount.fromRawAmount(native, JSBI.BigInt(value.toString()))
        return memo
      }, {}),
  ...
  return useMemo(
    () =>
      currencies?.map((currency) => {
        ...
        if (currency?.isNative) return nativeBalance[account.toLocaleLowerCase()]
        return undefined
      }) ?? [],
