...
import { cronos, bscTest, goerli, rinkeby } from '@pancakeswap/wagmi/chains'
...
const getNodeRealUrl = (networkName: string) => {
  ...
  switch (networkName) {
    ...
    case 'cronos':
      if (process.env.NEXT_PUBLIC_NODE_REAL_API_ETH) {
        host = `https://node.croswap.com/rpc`
      }
      break
    default:
      host = null
...
export const { provider, chains } = configureChains(CHAINS, [
  jsonRpcProvider({
    rpc: (chain) => {
      if (!!process.env.NEXT_PUBLIC_NODE_PRODUCTION && chain.id === cronos.id) {
...
export const defiWalletConnector = new DefiWalletConnector({ chains })
