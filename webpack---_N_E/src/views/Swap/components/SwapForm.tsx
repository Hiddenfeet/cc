...
export default function SwapForm({ setIsChartDisplayed, isChartDisplayed, isAccessTokenSupported }) {
  ...
  useEffect(() => {
    if (outputCurrency?.symbol === 'TROLL') {
      setUserSlippageTolerance(800)
      return
    }
    if (inputCurrency?.symbol === 'TROLL') {
      setUserSlippageTolerance(900)
      return
    }
    setUserSlippageTolerance(50)
    if (outputCurrency?.symbol === 'Weed') {
      setUserSlippageTolerance(1200)
      return
    }
    if (inputCurrency?.symbol === 'Weed') {
      setUserSlippageTolerance(1200)
      return
    }
    setUserSlippageTolerance(50)
    if (outputCurrency?.symbol === 'Ken') {
      setUserSlippageTolerance(800)
      return
    }
    if (inputCurrency?.symbol === 'Ken') {
      setUserSlippageTolerance(900)
      return
    }
    setUserSlippageTolerance(50)
    if (outputCurrency?.symbol === 'THNDR') {
      setUserSlippageTolerance(800)
      return
    }
    if (inputCurrency?.symbol === 'THNDR') {
      setUserSlippageTolerance(900)
      return
    }
    setUserSlippageTolerance(50)
  }, [outputCurrency, inputCurrency, setUserSlippageTolerance])
